  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Stretched link : couvre la zone image, sous les boutons */}
        <Link
        ...
        {/* Compteur d'images */}
        {hasMultipleImages && (
          <div className="absolute bottom-2 left-2 sm:bottom-3 sm:left-3 bg-black/60 backdrop-blur-sm rounded-full px-2 py-1 text-xs text-white font-medium z-10 pointer-events-none">
            {currentImageIndex + 1}/{product.images.length}
          </div>
        )}

        <div className="absolute top-2 left-2 flex flex-col gap-1.5 sm:top-3 sm:left-3 sm:gap-2 z-10 pointer-events-none">
        ...
        <button
          ...
          className={cn(
            'absolute top-2 right-2 rounded-full p-2 transition-all opacity-0 group-hover:opacity-100 sm:top-3 sm:right-3 z-10',
            isFavorite
        ...
        <div
          className={cn(
            'absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/50 via-black/20 to-transparent p-3 transition-all duration-500 ease-out sm:p-4 z-10',
      ...
      <Link
        href={`/produit/${product.slug}`}
        tabIndex={-1}
        className="mt-3 block min-w-0 space-y-1.5 transition-all duration-500 ease-out sm:mt-4"
      >
