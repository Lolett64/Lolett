...
export function NewsletterForm({ variant = 'light' }: NewsletterFormProps) {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      const res = await fetch('/api/newsletter/subscribe', {
        ...
        body: JSON.stringify({ email, source: 'editorial' }),
      });
      if (res.ok) {
        setStatus('success');
        setEmail('');
  ...
  const errorMessage =
    status === 'error' ? 'Une erreur est survenue, merci de réessayer.' : null;
  ...
  return (
    <div className="max-w-md mx-auto">
      ...
      {errorMessage && (
        <p className="text-[11px] uppercase tracking-[0.15em] text-red-500 mt-3 text-center">
          {errorMessage}
        </p>
      )}
    </div>
