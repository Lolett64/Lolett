...
export function NewArrivalsSection({ products, content, hexColor = '#FFFFFF' }: NewArrivalsSectionProps) {
      ...
      <div className="max-w-[1600px] mx-auto px-6 sm:px-10">
        ...
        <ScrollReveal className="mb-20">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-8 pb-8 border-b border-[#1B0B94]/8">
            <div className="flex items-start gap-6">
              <div>
                ...
                <h2 className="font-[family-name:var(--font-newsreader)] text-5xl sm:text-6xl lg:text-7xl italic text-[#1B0B94] tracking-tight leading-[0.95]">
                  {content?.title_line1 || 'Fraîchement'}<br className="hidden sm:block" /> {content?.title_line2 || 'Arrivées'}
                </h2>
              </div>
            </div>
            <Link
            ...
            >
              <span className="text-[9px] uppercase tracking-[0.25em] font-medium">{content?.see_all_cta || 'Voir toute la sélection'}</span>
              ...
              <div className="group relative">
                {i < 2 && (
                  <div className="absolute top-4 left-4 z-10 flex items-center gap-2">
                    ...
                    <span className="text-[8px] uppercase tracking-[0.2em] font-semibold text-[#B89547]">{content?.new_tag || 'New'}</span>
                  </div>
                )}
        ...
        <ScrollReveal className="mt-14 text-center md:hidden">
          ...
          >
            {content?.load_more_cta || 'Voir la suite'}
          </Link>
        </ScrollReveal>
      </div>
    </section>
