...
export default async function ShopPage() {
  const [femmeProducts, hommeProducts, accessoiresProducts, featuredLooks] = await Promise.all([
        ...
        <section className="bg-[#ebe5d9] py-16 sm:py-24">
          <div className="mx-auto max-w-[1600px] px-4 sm:px-6">
            ...
            <div className="grid grid-cols-1 gap-5 sm:grid-cols-3">
              {featuredLooks[0] && (
                <Link href={`/looks/${featuredLooks[0].id}`} className="group sm:col-span-2">
                  <div className="relative aspect-[16/10] overflow-hidden rounded-2xl bg-[#e5ddd2]">
                    <Image
                      src={featuredLooks[0].coverImage}
                      alt={featuredLooks[0].title}
                      fill
                    ...
                    <div className="absolute bottom-6 left-6 right-6">
                      <p className="text-[10px] font-medium tracking-wider uppercase text-[#B89547]">
                        {featuredLooks[0].gender === 'homme' ? 'Look Homme' : featuredLooks[0].gender === 'femme' ? 'Look Femme' : 'Look Mixte'}
                      </p>
                      <h3 className="font-display mt-1 text-2xl font-bold text-white">{featuredLooks[0].title}</h3>
                      <p className="mt-1 text-xs text-white/60">{featuredLooks[0].shortPitch}</p>
                    </div>
                  </div>
                </Link>
              )}
